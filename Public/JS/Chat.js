...
searchForm.addEventListener("submit", (Event) => {
    ...
    if (searchInput.value) {
        socket.emit("search friend", { query: searchInput.value, token });
        searchInput.value = "";
    }
});
...
async function openChatPanel(username) {
    ...
    if (!chatPanel) {
        ...
        // If chat panel doesn't exist, create a new one
        ...
        // Attach the event listener for the message form submit
    ...
    try {
        const response = await fetch(`/messages?user=${username}`, {
            method: "GET",
            headers: {
